...
def dashboard_admin2(request):
    ...
    if filter_type == '5days':
        ...
        labels = [(start_date + timedelta(weeks=i)).strftime('%Y-%W') for i in range(13)]  # รายสัปดาห์ (13 สัปดาห์)

    elif filter_type == '6months':
        start_date = today - timedelta(days=180)  # 6 เดือนล่าสุด
        labels = [(start_date + timedelta(weeks=i)).strftime('%Y-%W') for i in range(26)]  # รายสัปดาห์ (26 สัปดาห์)
    ...
    if filter_type in ['3months', '6months']:
        # กรณี 3 เดือน หรือ 6 เดือน ให้คำนวณรายได้รายสัปดาห์
        num_weeks = 13 if filter_type == '3months' else 26  # 13 สัปดาห์สำหรับ 3 เดือน, 26 สัปดาห์สำหรับ 6 เดือน
        labels = [(start_date + timedelta(weeks=i)).strftime('%Y-%W') for i in range(num_weeks)]  # สร้าง labels เป็นรายสัปดาห์
        ...
        # กรณีอื่น ๆ ให้คำนวณรายได้รายวัน
        for label in labels:
...
def dashboard_admin(request):
    today = timezone.localtime(timezone.now()).date()
    shop_id = request.session.get('shop_id', 1)
