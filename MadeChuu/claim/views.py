...
def claim_request(request, order_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            claim.order = order
            claim.claim_status = get_object_or_404(ClaimStatus, claim_status_name="กำลังดำเนินการ")
            claim.save()

            # เปลี่ยนสถานะออเดอร์เป็น "เคลม" หลังจากยื่นคำขอสำเร็จ
            claim_status = get_object_or_404(StatusOrder, status_name="เคลม")
            order.status_order = claim_status
            order.save()
