...
class Product(models.Model):
    ...
    def get_discounted_price(self):
        promotion = self.promotionproduct_set.first()
        if promotion:
...
class Shop(models.Model):
    ...
    phone_num = models.CharField(max_length=10)
    qr_code = models.ImageField(upload_to='qr_codes/', blank=True, null=True)  # เพิ่มฟิลด์สำหรับเก็บ QR Code
...
class CartItem(models.Model):
    ...
    def total_price(self):
        """คำนวณราคารวมอัตโนมัติเมื่อเรียกใช้"""
        promotion = self.product.promotionproduct_set.first()
        if promotion:
            discount = promotion.promotion_id.discount / 100
            return self.quantity * (self.product.price * (1 - discount))
...
class OrderProduct(models.Model):
    ...
    product = models.ForeignKey('Product', on_delete=models.CASCADE)
    quantity = models.IntegerField()
    price = models.FloatField(blank=True, null=True)  # เพิ่มฟิลด์ price
    discount = models.FloatField(default=0)  # เพิ่มฟิลด์ discount
